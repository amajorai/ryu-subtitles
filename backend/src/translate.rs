...
pub async fn translate_cues<F, Fut>(
...
mod tests {
    ...
    #[tokio::test]
    async fn cancellation_is_checked_before_the_first_gateway_call() {
        let mut cues = vec![cue("one")];
        let result = translate_cues(
            ...
            crate::languages::find("en").expect("default language is registered"),
            &mut cues,
            || async { true },
            |_, _| {},
        )
        ...
        assert!(cues[0].translated.is_none());
    }
    ...
    fn gateway_config_prefers_an_explicit_model_over_the_bundled_default() {
        ...
        assert!(
            !blank.model.is_empty(),
            "blank must fall through, not stick"
        );
    }
}
