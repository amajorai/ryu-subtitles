...
mod tests {
    ...
    fn scratch(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir()
            .join(format!("ryu-subtitles-lib-{}-{tag}", std::process::id()));
    ...
    fn a_path_outside_every_root_is_refused() {
        // `/etc/hosts` exists on every platform this ships to and is outside home.
        let err = validate_source("/etc/hosts").expect_err("must refuse");
    ...
    fn traversal_out_of_a_root_is_refused_after_canonicalization() {
        ...
        let escape = format!("{}/../../../../etc/hosts", home.display());
    ...
    fn a_media_file_under_home_validates_and_comes_back_canonical() {
        ...
        std::fs::write(&path, b"not really a wav, but the gate is extension-based")
            .expect("write");
    ...
    fn listing_a_directory_outside_the_roots_is_refused() {
        let err = list_dir(Path::new("/etc")).expect_err("must refuse");
