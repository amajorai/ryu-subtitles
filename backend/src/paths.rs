...
//! `apps-store/social/backend/src/paths.rs` and every other apps-store sidecar).
//!
//! The sidecar MUST resolve the SAME data dir Core uses so it opens the SAME
//! `subtitles.db` the node uses, and writes its generated cue files under the SAME
//! `subtitles/` output directory the desktop downloads from. The
