...
impl SubtitleStore {
    ...
    pub async fn cues(&self, id: &str) -> Result<Vec<Cue>> {
        ...
        let json: Option<String> = conn
            .query_row("SELECT cues FROM jobs WHERE id = ?1", params![id], |r| r.get(0))
            .optional()?;
...
mod tests {
    ...
    async fn jobs_list_newest_first_and_respect_the_limit() {
        let store = SubtitleStore::memory().expect("store");
        for i in 0..3 {
            store.create_job(new_job(&format!("f{i}"))).await.expect("create");
    ...
    async fn progress_is_clamped_and_duration_is_sticky() {
        ...
        store.set_progress(&job.id, 0.5, "Translating", None).await.expect("progress");
        let loaded = store.get_job(&job.id).await.expect("get").expect("present");
        assert_eq!(loaded.duration_ms, Some(120_000));
    }
    ...
    async fn cues_round_trip_and_drive_the_counts() {
        ...
        let job = store.create_job(new_job("film")).await.expect("create");
        let cues = vec![
            cue(0, "hello", Some("hola")),
            cue(2_000, "goodbye", None),
        ];
    ...
    async fn terminal_status_stamps_completed_at_once() {
        let store = SubtitleStore::memory().expect("store");
        let job = store.create_job(new_job("film")).await.expect("create");
        store.set_status(&job.id, Status::Completed, None).await.expect("status");
        ...
        assert_eq!(second.completed_at, first.completed_at, "stamp must not move");
    ...
    async fn requeue_clears_the_previous_outcome() {
        let store = SubtitleStore::memory().expect("store");
        let job = store.create_job(new_job("film")).await.expect("create");
        store.set_cues(&job.id, &[cue(0, "hi", Some("hola"))]).await.expect("cues");
        store.set_status(&job.id, Status::Failed, Some("boom")).await.expect("status");
    ...
    async fn next_queued_is_oldest_first_and_skips_running_jobs() {
        ...
        assert_eq!(store.next_queued().await.expect("next").map(|j| j.id), Some(first.id.clone()));
        store.set_status(&first.id, Status::Transcribing, None).await.expect("status");
        assert_eq!(store.next_queued().await.expect("next").map(|j| j.id), Some(second.id));
    }
    ...
    async fn a_restart_cancels_jobs_that_claimed_to_be_running() {
        ...
        store.set_status(&running.id, Status::Transcribing, None).await.expect("status");

        assert_eq!(store.reset_interrupted().await.expect("reset"), 1);
        let after = store.get_job(&running.id).await.expect("get").expect("present");
        ...
        let untouched = store.get_job(&queued.id).await.expect("get").expect("present");
