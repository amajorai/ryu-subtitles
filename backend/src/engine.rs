            ...
            match worker.store.next_queued().await {
                Ok(Some(job)) => {
                    ...
                    if let Err(e) = run_job(&worker, job).await {
                        ...
                        let _ = worker
                            .store
                            .set_status(&id, Status::Failed, Some(&message))
                            .await;
                        worker
                            .events
                            .emit(
...
async fn run_job(worker: &Worker, job: Job) -> anyhow::Result<()> {
    ...
    let id = job.id.clone();
    store.set_status(&id, Status::Transcribing, None).await?;
    store
    ...
    library::validate_source(&job.source_path)
    ...
    if needs_translation {
        store.set_status(&id, Status::Translating, None).await?;
    ...
    if settings.write_beside_source {
        let target = library::sidecar_output_path(
            &source,
            &job.target_language,
    ...
    store
        .set_progress(&id, 1.0, "Done", None)
        .await?;
    store.set_status(&id, Status::Completed, None).await?;
