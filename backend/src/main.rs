...
async fn main() -> anyhow::Result<()> {
    ...
    // derives one LLM tool per operation. Root, not under the mount, because that is
    // the only address the fetcher tries; gated, not next to `/health`, because the
    // document names every route and body field this app accepts.
